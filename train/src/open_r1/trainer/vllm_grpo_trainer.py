...
class Qwen2VLGRPOVLLMTrainer(Trainer):
        ...
        if isinstance(model, str):
            ...
            if "Qwen2-VL" in model_id:
                model = Qwen2VLForConditionalGeneration.from_pretrained(
                    model, **model_init_kwargs, cache_dir=cache_dir
                )
        ...
        if is_deepspeed_zero3_enabled():
            if "Qwen2-VL" in model_id:
                self.ref_model = Qwen2VLForConditionalGeneration.from_pretrained(
                    model_id, **model_init_kwargs, cache_dir=cache_dir
                )
            elif "Aria" in model_id:
    ...
    def _get_per_token_logps(
        ...
        video_grid_thw = video_grid_thw.to(device=model.device)
        logits = model(
            input_ids,
    ...
    def _prepare_inputs(
        ...
        if self.args.use_vllm:
            ...
            all_multimodal_inputs = [
                ...
                for p, v in zip(all_prompts_text, all_videos)
            ]
        ...
        # 正确处理 pixel_values 的维度
        pixel_values_videos = prompt_inputs["pixel_values_videos"]
        pixel_values_videos = pixel_values_videos.unsqueeze(1).repeat_interleave(self.num_generations, dim=1)
        pixel_values_videos = pixel_values_videos.view(-1, *pixel_values_videos.shape[2:])

        video_grid_thw = prompt_inputs["video_grid_thw"]
        video_grid_thw = video_grid_thw.unsqueeze(1).repeat_interleave(self.num_generations, dim=1)
        video_grid_thw = video_grid_thw.view(-1, *video_grid_thw.shape[2:])
